...
去中心化初始化的过程可以通过使用 [Atomicals 命令行工具](../reference-tools/javascript-library-cli) 来完成，其基本使用格式如下：
...
- `--mintbitworkc=<prefix>`：定义铸造时可选的 [Bitwork](../bitwork-mining) 挖矿难度前缀。设置后，铸造者需通过计算找到一个匹配该前缀的哈希值才能成功铸造，类似于执行比特币的工作量证明过程。建议选择4到6位的十六进制前缀。前缀中的字符可以是 `a-f` 和 `0-9` 范围内的任何有效十六进制数字，选择哪个字符完全取决于个人偏好。
...
使用 [Atomicals 命令行工具](../reference-tools/javascript-library-cli) 进行去中心化铸造的基本格式如下：
...
利用[永久文件存储](../permanent-file-storage)功能，您可以将图像等资料直接存储于区块链上，并在下述的 `image` 字段中进行引用。
